    ...
    <section id="certificates" className="py-24 bg-[var(--surface)] relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative">
        <div className="flex items-center gap-4 mb-4">
          <motion.p className="text-[var(--lime)] text-xs font-mono tracking-[0.3em] uppercase"
        ...
        {selected && (
          ...
          >
            <motion.div
              ...
              className="relative max-w-3xl w-full bg-black border border-[var(--lime)]/20 rounded-2xl p-4 sm:p-6 shadow-[0_0_60px_rgba(0,0,0,0.6)]"
            >
              <div className="flex justify-between items-center mb-4 gap-4">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <h3 className="text-white font-bold truncate">{t(selected.titleKey)}</h3>
                </div>
              ...
              <div className="relative">
                <img src={selected.image} alt={t(selected.titleKey)}
                  className="w-full max-h-[70vh] object-contain rounded-xl" />

                <button
                  onClick={() => go(-1)}
                  aria-label="Anterior"
                  className="absolute left-2 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-black/70 border border-white/10 text-white hover:border-[var(--lime)] hover:text-[var(--lime)] transition-colors flex items-center justify-center"
                >
                  ‹
                </button>
                <button
                  onClick={() => go(1)}
                  aria-label="Seguinte"
                  className="absolute right-2 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-black/70 border border-white/10 text-white hover:border-[var(--lime)] hover:text-[var(--lime)] transition-colors flex items-center justify-center"
                >
                  ›
                </button>
              </div>
