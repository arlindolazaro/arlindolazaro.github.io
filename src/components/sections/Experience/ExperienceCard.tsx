...
export const ExperienceCard = ({ experience, index }: Props) => {
  ...
  return (
    ...
    >
      {/* Timeline dot — ponto preto fixo intencional para contraste dentro do círculo lime em ambos os temas */}
      ...
      <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-6 sm:p-8 hover:border-[var(--lime)]/30 transition-colors duration-300">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4">
          <div>
            <h3 className="text-[var(--text)] font-bold text-lg">{experience.position[lang]}</h3>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {experience.techStack.map((tech, i) => (
            <span
              key={i}
              className="px-2 py-1 text-xs rounded-md bg-[var(--lime)]/10 text-[var(--lime)] border border-[var(--lime)]/20"
            >
              {tech}
            </span>
          ))}
